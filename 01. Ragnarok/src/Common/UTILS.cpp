...
namespace UTIL
{
	...
	void PrintText(HDC _hdc, const char* _str, char* _font, int _bold,int _destX, int _destY,
		int _fontSize, COLORREF _fontColor, bool _isBgTransParent, COLORREF _bgColor)
	{
		HFONT curFont, oldFont;
		COLORREF oldBkColor, oldFontColor;
		...
		curFont = CreateFont(_fontSize, 0, 0, 0, _bold,
			FALSE, FALSE, FALSE, DEFAULT_CHARSET,
			OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY,
			FF_DONTCARE, _font
		);
	...
	bool isPointPointCollision(const POINT& point, const POINT& point2)
	{
